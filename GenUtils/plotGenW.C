...
void plotGenW(const TString input) 
{
  gBenchmark->Start("plotGenW");
  ...
  vector<TH1F*> hWMass1v;
  vector<TH1F*> hWMass2v;
  vector<TH1F*> hWPtv;   
  vector<TH1F*> hWyv;    
  vector<TH1F*> hWPhiv;  
  ...
  for(UInt_t ifile=0; ifile<fnamev.size(); ifile++) {
    ...
    sprintf(hname,"hWMass1_%i",ifile); hWMass1v.push_back(new TH1F(hname,"",100,0,1000)); hWMass1v[ifile]->Sumw2();
    sprintf(hname,"hWMass2_%i",ifile); hWMass2v.push_back(new TH1F(hname,"",100,0,200));  hWMass2v[ifile]->Sumw2();
    sprintf(hname,"hWPt_%i",ifile);    hWPtv.push_back(new TH1F(hname,"",50,0,100));      hWPtv[ifile]->Sumw2();
    sprintf(hname,"hWy_%i",ifile);     hWyv.push_back(new TH1F(hname,"",80,-8,8));        hWyv[ifile]->Sumw2();
    sprintf(hname,"hWPhi_%i",ifile);   hWPhiv.push_back(new TH1F(hname,"",80,-3.2,3.2));  hWPhiv[ifile]->Sumw2();

    sprintf(hname,"hMt1_%i",ifile);           hMt1v.push_back(new TH1F(hname,"",100,0,1000));           hMt1v[ifile]->Sumw2();
    sprintf(hname,"hMt2_%i",ifile);           hMt2v.push_back(new TH1F(hname,"",100,0,200));            hMt2v[ifile]->Sumw2();
  ...
  for(UInt_t ifile=0; ifile<fnamev.size(); ifile++) {
    ...
    for(UInt_t ientry=0; ientry<eventTree->GetEntries(); ientry++) {
      ...
      if((data.pt_1>20) && (data.pt_2>20) && (fabs(data.eta_1)<2.1) && (fabs(data.eta_2)<2.1) )
        nPassv[ifile] += data.weight;
      
      hScale1v[ifile]->Fill(data.scalePdf,data.weight);
      hScale2v[ifile]->Fill(data.scalePdf,data.weight);
      
      hWMass1v[ifile]->Fill(data.vmass,data.weight);
      hWMass2v[ifile]->Fill(data.vmass,data.weight);
      hWPtv[ifile]   ->Fill(data.vpt,  data.weight);
      hWyv[ifile]    ->Fill(data.vy,   data.weight);
      hWPhiv[ifile]  ->Fill(data.vphi, data.weight);
    
      hMt1v[ifile]          ->Fill(data.mt,  data.weight);
      hMt2v[ifile]          ->Fill(data.mt,  data.weight);
  ...
  // W mass
  sprintf(ylabel,"a.u. / %.1f GeV/c^{2}",hWMass1v[0]->GetBinWidth(1));
  CPlot plotWMass1("wmass1","","m(W^{-}) [GeV/c^{2}]",ylabel);
  for(UInt_t i=0; i<fnamev.size(); i++) { 
    hWMass1v[i]->Scale(1.0/(Double_t)nEventsv[i]);
    plotWMass1.AddHist1D(hWMass1v[i],labelv[i],"hist",colorv[i],linev[i]); 
  }
  plotWMass1.SetLogy();
  plotWMass1.Draw(c,kTRUE,format);
    
  sprintf(ylabel,"a.u. / %.1f GeV/c^{2}",hWMass2v[0]->GetBinWidth(1));
  CPlot plotWMass2("wmass2","","m(W^{-}) [GeV/c^{2}]",ylabel);
  for(UInt_t i=0; i<fnamev.size(); i++) { 
    hWMass2v[i]->Scale(1.0/(Double_t)nEventsv[i]);
    plotWMass2.AddHist1D(hWMass2v[i],labelv[i],"hist",colorv[i],linev[i]); 
  }
  plotWMass2.SetLogy();
  plotWMass2.Draw(c,kTRUE,format);
        
  // W pT
  sprintf(ylabel,"a.u. / %.1f GeV/c",hWPtv[0]->GetBinWidth(1));
  CPlot plotWPt("wpt","","p_{T}(W^{-}) [GeV/c]",ylabel);
  for(UInt_t i=0; i<fnamev.size(); i++) { 
    hWPtv[i]->Scale(1.0/(Double_t)nEventsv[i]);
    plotWPt.AddHist1D(hWPtv[i],labelv[i],"hist",colorv[i],linev[i]);
  }
  plotWPt.Draw(c,kTRUE,format);
  
  // W rapidity
  sprintf(ylabel,"a.u. / %.2f",hWyv[0]->GetBinWidth(1));
  CPlot plotWy("wy","","y(W^{-})",ylabel);
  for(UInt_t i=0; i<fnamev.size(); i++) { 
    hWyv[i]->Scale(1.0/(Double_t)nEventsv[i]);
    plotWy.AddHist1D(hWyv[i],labelv[i],"hist",colorv[i],linev[i]); 
  }
  plotWy.TransLegend(0.1,0);
  plotWy.Draw(c,kTRUE,format);
  
  // W phi
  sprintf(ylabel,"a.u. / %.2f",hWPhiv[0]->GetBinWidth(1));
  CPlot plotWPhi("wphi","","#phi(W^{-})",ylabel);
  for(UInt_t i=0; i<fnamev.size(); i++) { 
    hWPhiv[i]->Scale(1.0/(Double_t)nEventsv[i]);
    plotWPhi.AddHist1D(hWPhiv[i],labelv[i],"hist",colorv[i],linev[i]); 
  }
//  plotWPhi.SetYRange(0.8*(hWPhiv[0]->GetMaximum()),1.2*(hWPhiv[0]->GetMaximum()));
  plotWPhi.Draw(c,kTRUE,format); 

  // transverse mass
  sprintf(ylabel,"a.u. / %.1f GeV/c^{2}",hMt1v[0]->GetBinWidth(1));
  CPlot plotMt1("mt1","","m_{T}(l^{-}#bar{#nu}) [GeV/c^{2}]",ylabel);
  for(UInt_t i=0; i<fnamev.size(); i++) { 
    hMt1v[i]->Scale(1.0/(Double_t)nEventsv[i]);
    plotMt1.AddHist1D(hMt1v[i],labelv[i],"hist",colorv[i],linev[i]); 
  }
  plotMt1.SetLogy();
  plotMt1.Draw(c,kTRUE,format);
  
  sprintf(ylabel,"a.u. / %.1f GeV/c^{2}",hMt2v[0]->GetBinWidth(1));
  CPlot plotMt2("mt2","","m_{T}(l^{-}#bar{#nu}) [GeV/c^{2}]",ylabel);
  for(UInt_t i=0; i<fnamev.size(); i++) { 
    hMt2v[i]->Scale(1.0/(Double_t)nEventsv[i]);
    plotMt2.AddHist1D(hMt2v[i],labelv[i],"hist",colorv[i],linev[i]); 
  }
  plotMt2.SetLogy();
  plotMt2.Draw(c,kTRUE,format);
  
  // dilepton mass
  sprintf(ylabel,"a.u. / %.1f GeV/c^{2}",hDileptonMass1v[0]->GetBinWidth(1));
  CPlot plotDileptonMass1("dileptonmass1","","m(l^{-}#bar{#nu}) [GeV/c^{2}]",ylabel);
  for(UInt_t i=0; i<fnamev.size(); i++) { 
  ...
  CPlot plotDileptonMass2("dileptonmass2","","m(l^{-}#bar{#nu}) [GeV/c^{2}]",ylabel);
  for(UInt_t i=0; i<fnamev.size(); i++) { 
  ...
  CPlot plotDileptonPt("dileptonpt","","p_{T}(l^{-}#bar{#nu}) [GeV/c]",ylabel);
  for(UInt_t i=0; i<fnamev.size(); i++) { 
  ...
  CPlot plotDileptony("dileptony","","y(l^{-}#bar{#nu})",ylabel);
  for(UInt_t i=0; i<fnamev.size(); i++) { 
  ...
  CPlot plotDileptonPhi("dileptonphi","","#phi(l^{-}#bar{#nu})",ylabel);
  for(UInt_t i=0; i<fnamev.size(); i++) { 
...
//  plotDileptonPhi.SetYRange(0.8*(hDileptonPhiv[0]->GetMaximum()),1.2*(hDileptonPhiv[0]->GetMaximum()));
  ...
  CPlot plotLepton2Pt("mu2pt","","p_{T}(#bar{#nu}) [GeV/c]",ylabel);
  for(UInt_t i=0; i<fnamev.size(); i++) { 
  ...
  CPlot plotLepton2Eta("mu2eta","","#eta(#bar{#nu})",ylabel);
  for(UInt_t i=0; i<fnamev.size(); i++) { 
  ...
  CPlot plotLepton2Phi("mu2phi","","#phi(#bar{#nu})",ylabel);
  for(UInt_t i=0; i<fnamev.size(); i++) { 
  ...
  // W mass
  CPlot plotWMassRatio1("wmassRatio1","","m(W^{-}) [GeV/c^{2}]","ratio");
  for(UInt_t i=1; i<fnamev.size(); i++) { 
    TGraphErrors* ratio = makeRatioGraph(hWMass1v[0],hWMass1v[i]);
    plotWMassRatio1.AddGraph(ratio,labelv[i],"XL",colorv[i],kFullDotMedium,1); 
  }
  plotWMassRatio1.SetYRange(0.5,1.5);
  plotWMassRatio1.AddLine(0,1,1100,1,kBlack,2);
  plotWMassRatio1.Draw(c,kTRUE,format);
    
  CPlot plotWMassRatio2("wmassRatio2","","m(W^{-}) [GeV/c^{2}]","ratio");
  for(UInt_t i=1; i<fnamev.size(); i++) { 
    TGraphErrors* ratio = makeRatioGraph(hWMass2v[0],hWMass2v[i]);
    plotWMassRatio2.AddGraph(ratio,labelv[i],"XL",colorv[i],kFullDotMedium,1); 
  }
  plotWMassRatio2.SetYRange(0.5,1.5);
  plotWMassRatio2.AddLine(0,1,220,1,kBlack,2);
  plotWMassRatio2.Draw(c,kTRUE,format);
        
  // W pT
  CPlot plotWPtRatio("wptRatio","","p_{T}(W^{-}) [GeV/c]","ratio");
  for(UInt_t i=1; i<fnamev.size(); i++) { 
    TGraphErrors* ratio = makeRatioGraph(hWPtv[0],hWPtv[i]);
    plotWPtRatio.AddGraph(ratio,labelv[i],"XL",colorv[i],kFullDotMedium,1); 
  }
  plotWPtRatio.SetYRange(0.5,1.5);
  plotWPtRatio.AddLine(0,1,110,1,kBlack,2);
  plotWPtRatio.Draw(c,kTRUE,format);
  
  // W rapidity
  CPlot plotWyRatio("wyRatio","","y(W^{-})","ratio");
  for(UInt_t i=1; i<fnamev.size(); i++) { 
    TGraphErrors* ratio = makeRatioGraph(hWyv[0],hWyv[i]);
    plotWyRatio.AddGraph(ratio,labelv[i],"XL",colorv[i],kFullDotMedium,1); 
  }
  plotWyRatio.SetYRange(0.5,1.5);
  plotWyRatio.AddLine(-9.5,1,9.5,1,kBlack,2);
  plotWyRatio.Draw(c,kTRUE,format);

  // Transverse mass
  CPlot plotMtRatio1("mtRatio1","","m_{T}(l^{-}#bar{#nu}) [GeV/c^{2}]","ratio");
  for(UInt_t i=1; i<fnamev.size(); i++) { 
    TGraphErrors* ratio = makeRatioGraph(hMt1v[0],hMt1v[i]);
    plotMtRatio1.AddGraph(ratio,labelv[i],"XL",colorv[i],kFullDotMedium,1); 
  }
  plotMtRatio1.SetYRange(0.5,1.5);
  plotMtRatio1.AddLine(0,1,1100,1,kBlack,2);
  plotMtRatio1.Draw(c,kTRUE,format);
    
  CPlot plotMtRatio2("mtRatio2","","m_{T}(l^{-}#bar{#nu}) [GeV/c^{2}]","ratio");
  for(UInt_t i=1; i<fnamev.size(); i++) { 
    TGraphErrors* ratio = makeRatioGraph(hMt2v[0],hMt2v[i]);
    plotMtRatio2.AddGraph(ratio,labelv[i],"XL",colorv[i],kFullDotMedium,1); 
  }
  plotMtRatio2.SetYRange(0.5,1.5);
  plotMtRatio2.AddLine(0,1,220,1,kBlack,2);
  plotMtRatio2.Draw(c,kTRUE,format);
  
  // Dilepton mass
  CPlot plotDileptonMassRatio1("dileptonmassRatio1","","m(l^{-}#bar{#nu}) [GeV/c^{2}]","ratio");
  for(UInt_t i=1; i<fnamev.size(); i++) { 
  ...
  CPlot plotDileptonMassRatio2("dileptonmassRatio2","","m(l^{-}#bar{#nu}) [GeV/c^{2}]","ratio");
  for(UInt_t i=1; i<fnamev.size(); i++) { 
  ...
  // Dilepton pT
  CPlot plotDileptonPtRatio("dileptonptRatio","","p_{T}(l^{-}#bar{#nu}) [GeV/c]","ratio");
  for(UInt_t i=1; i<fnamev.size(); i++) { 
  ...
  // Dilepton rapidity
  CPlot plotDileptonyRatio("dileptonyRatio","","y(l^{-}#bar{#nu})","ratio");
  for(UInt_t i=1; i<fnamev.size(); i++) { 
  ...
  // neutrino pT
  CPlot plotLepton2PtRatio("mu2ptRatio","","p_{T}(#bar{#nu}) [GeV/c]","ratio");
  for(UInt_t i=1; i<fnamev.size(); i++) { 
  ...
  // neutrino eta 
  CPlot plotLepton2EtaRatio("mu2etaRatio","","#eta(#bar{#nu})","ratio");
  for(UInt_t i=1; i<fnamev.size(); i++) { 
...
void makeHTML(const TString outDir)
{
  ...
  htmlfile << "<table border=\"0\" cellspacing=\"5\" width=\"100%\">" << endl;
  htmlfile << "<tr>" << endl;
  htmlfile << "<td width=\"25%\"><a target=\"_blank\" href=\"gen/wmass1.png\"><img src=\"gen/wmass1.png\" alt=\"gen/wmass1.png\" width=\"100%\"></a></td>" << endl;
  htmlfile << "<td width=\"25%\"><a target=\"_blank\" href=\"gen/wmass2.png\"><img src=\"gen/wmass2.png\" alt=\"gen/wmass2.png\" width=\"100%\"></a></td>" << endl;
  htmlfile << "<td width=\"25%\"></td>" << endl;
  htmlfile << "<td width=\"25%\"></td>" << endl;
  htmlfile << "</tr>" << endl;   
  htmlfile << "<tr>" << endl;
  htmlfile << "<td width=\"25%\"><a target=\"_blank\" href=\"gen/wmassRatio1.png\"><img src=\"gen/wmassRatio1.png\" alt=\"gen/wmassRatio1.png\" width=\"100%\"></a></td>" << endl;
  htmlfile << "<td width=\"25%\"><a target=\"_blank\" href=\"gen/wmassRatio2.png\"><img src=\"gen/wmassRatio2.png\" alt=\"gen/wmassRatio2.png\" width=\"100%\"></a></td>" << endl;
  htmlfile << "<td width=\"25%\"></td>" << endl;
  htmlfile << "<td width=\"25%\"></td>" << endl;
  htmlfile << "</tr>" << endl;
  htmlfile << "<tr>" << endl;
  htmlfile << "<td width=\"25%\"><a target=\"_blank\" href=\"gen/wpt.png\"><img src=\"gen/wpt.png\" alt=\"gen/wpt.png\" width=\"100%\"></a></td>" << endl;
  htmlfile << "<td width=\"25%\"><a target=\"_blank\" href=\"gen/wy.png\"><img src=\"gen/wy.png\" alt=\"gen/wy.png\" width=\"100%\"></a></td>" << endl;
  htmlfile << "<td width=\"25%\"><a target=\"_blank\" href=\"gen/wphi.png\"><img src=\"gen/wphi.png\" alt=\"gen/wphi.png\" width=\"100%\"></a></td>" << endl;
  htmlfile << "<td width=\"25%\"></td>" << endl;
  htmlfile << "</tr>" << endl;
  htmlfile << "<tr>" << endl;
  htmlfile << "<td width=\"25%\"><a target=\"_blank\" href=\"gen/wptRatio.png\"><img src=\"gen/wptRatio.png\" alt=\"gen/wptRatio.png\" width=\"100%\"></a></td>" << endl;
  htmlfile << "<td width=\"25%\"><a target=\"_blank\" href=\"gen/wyRatio.png\"><img src=\"gen/wyRatio.png\" alt=\"gen/wyRatio.png\" width=\"100%\"></a></td>" << endl;
  htmlfile << "<td width=\"25%\"></td>" << endl;
  htmlfile << "<td width=\"25%\"></td>" << endl;
  htmlfile << "</tr>" << endl;
  htmlfile << "</table>" << endl;
  ...
  htmlfile << "<table border=\"0\" cellspacing=\"5\" width=\"100%\">" << endl;
  htmlfile << "<tr>" << endl;
  htmlfile << "<td width=\"25%\"><a target=\"_blank\" href=\"gen/mt1.png\"><img src=\"gen/mt1.png\" alt=\"gen/mt1.png\" width=\"100%\"></a></td>" << endl;
  htmlfile << "<td width=\"25%\"><a target=\"_blank\" href=\"gen/mt2.png\"><img src=\"gen/mt2.png\" alt=\"gen/mt2.png\" width=\"100%\"></a></td>" << endl;
  ...
  htmlfile << "</tr>" << endl;
  htmlfile << "<tr>" << endl;
  htmlfile << "<td width=\"25%\"><a target=\"_blank\" href=\"gen/mtRatio1.png\"><img src=\"gen/mtRatio1.png\" alt=\"gen/mtRatio1.png\" width=\"100%\"></a></td>" << endl;
  htmlfile << "<td width=\"25%\"><a target=\"_blank\" href=\"gen/mtRatio2.png\"><img src=\"gen/mtRatio2.png\" alt=\"gen/mtRatio2.png\" width=\"100%\"></a></td>" << endl;
