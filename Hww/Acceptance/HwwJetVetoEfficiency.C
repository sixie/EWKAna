//root -l EWKAna/Hww/Acceptance/HwwJetVetoEfficiency.C+\(\"/home/sixie/hist/WWAnalysis/WWAnalysisSkimmed_full_noskim.root\",\"\"\)
//root -l EWKAna/Hww/Acceptance/HwwJetVetoEfficiency.C+\(\"HwwNtuple_f10-ww2l-z2-v12-pu_noskim_0000.root\",\"WW\"\)
//root -l EWKAna/Hww/Acceptance/HwwJetVetoEfficiency.C+\(\"HwwNtuple_f10-h160ww2l-gf-z2-v12_noskim_0000.root\",\"HWW\"\)
//================================================================================================
//
// HWW selection macro
//
...
void HwwJetVetoEfficiency(const string inputFilename, 
                 const string Label) 
{  
...
//   ChargeSelection = 1;
  ...
  // Histograms
  //==============================================================================================================  
  TH1D *leadingJetPtHist = new TH1D((string("leadingJetPtHist") + label).c_str(), "; Leading Jet p_{T} [GeV/c]; Number of Events", 200, 0, 200);
  ...
  for(UInt_t ientry=0; ientry<eventTree->GetEntries(); ientry++) {       	
    ...
    for(int i = 0; i < leptonPt.size(); ++i) {
      for(int j = i+1; j < leptonPt.size(); ++j) {
        ...
        //*********************************************************************************************  
        if (passCut[0] && passCut[1] && met.Pt() > 30) {
  ...
  cv->SaveAs((string("leadingJetPt") + label + ".gif").c_str());

  jetVetoEfficiency->Draw();
  cv->SaveAs((string("JetVetoEfficiency") + label+".gif").c_str());
