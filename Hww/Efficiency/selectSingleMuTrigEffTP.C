...
void selectSingleMuTrigEffTP(const TString conf,              // input file
  ...
  for(UInt_t ifile=0; ifile<fnamev.size(); ifile++) {  
	...
	for(Int_t j=0; j<muonArr->GetEntriesFast(); j++) {
	  ...
	  Bool_t pass = kFALSE;
	  if(info->runNum >= 150000 && info->runNum <= 163261) pass = pass || ((info->triggerBits & kHLT_Mu15)           && (probe->hltMatchBits & kHLTObject_Mu15));	  
// 	  if(info->runNum >= 163262 && info->runNum <= 164237) pass = pass || ((info->triggerBits & kHLT_Mu24)           && (probe->hltMatchBits & kHLTObject_Mu24));  //i don't have this in the ntuple...so use Mu30 instead
	  if(info->runNum >= 163262 && info->runNum <= 164237) pass = pass || ((info->triggerBits & kHLT_Mu30)           && (probe->hltMatchBits & kHLTObject_Mu30));  
	  if(info->runNum >= 165085 && info->runNum <= 170053) pass = pass || ((info->triggerBits & kHLT_Mu30)           && (probe->hltMatchBits & kHLTObject_Mu30));
	  if(info->runNum >= 170054                          ) pass = pass || ((info->triggerBits & kHLT_Mu30)           && (probe->hltMatchBits & kHLTObject_Mu40));

	  if(info->runNum >= 163262 && info->runNum <= 170053) pass = pass || ((info->triggerBits & kHLT_IsoMu17)        && (probe->hltMatchBits & kHLTObject_IsoMu17));	  
	  if(info->runNum >= 170054 && info->runNum <= 173198) pass = pass || ((info->triggerBits & kHLT_IsoMu24)        && (probe->hltMatchBits & kHLTObject_IsoMu24));	  
	  if(info->runNum >= 173199)                           pass = pass || ((info->triggerBits & kHLT_IsoMu30)        && (probe->hltMatchBits & kHLTObject_IsoMu30));
	  if(typev[ifile]==eMC) 
	    pass = ((info->triggerBits & kHLT_Mu15) && (probe->hltMatchBits & kHLTObject_Mu15)); 
