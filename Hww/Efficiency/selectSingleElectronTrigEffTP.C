...
void selectSingleElectronTrigEffTP(const TString conf,              // input file
                                   const TString outputDir,         // output directory
                                   Int_t RunRange = 0,              // Run Range
                                   const Bool_t  matchGen = kFALSE  // match to generator electrons
) {
  gBenchmark->Start("selectSingleElectronTrigEffTP");
  ...
  for(UInt_t ifile=0; ifile<fnamev.size(); ifile++) {  
    ...
    for(UInt_t ientry=0; ientry<eventTree->GetEntries(); ientry++) {
      ...
      if (RunRange == 1) {
        ...
        if (!(info->runNum >= 165085 && info->runNum <= 166967)) passRunRange = kFALSE;
      } else if (RunRange == 3) {
        if (!(info->runNum >= 166968 && info->runNum <= 170053)) passRunRange = kFALSE;
      } else if (RunRange == 4) {
        if (!(info->runNum >= 170054 && info->runNum <= 178380)) passRunRange = kFALSE;
      } else if (RunRange == 5) {
        if (!(info->runNum >= 178381 && info->runNum <= 999999)) passRunRange = kFALSE;
      } else if (RunRange == 10) {
        if (!(info->runNum >= 0 && info->runNum <= 166967)) passRunRange = kFALSE;
      } else if (RunRange == 11) {
        if (!(info->runNum >= 0 && info->runNum <= 170053)) passRunRange = kFALSE;
      } else if (RunRange == 12) {
        if (!(info->runNum >= 0 && info->runNum <= 178380)) passRunRange = kFALSE;
      }
	...
	for(Int_t j=0; j<electronArr->GetEntriesFast(); j++) {
	  ...
	  nProbes += weight;
          ...
          ULong_t myTrigBit = 0;
          ULong_t myTrigObj = 0;
          if (info->runNum <= 164237) {
            myTrigObj = kHLTObject_Ele27_CaloIdVT_CaloIsoT_TrkIdT_TrkIsoT;          
            myTrigBit = kHLT_Ele27_CaloIdVT_CaloIsoT_TrkIdT_TrkIsoT;
          } else if (info->runNum >= 165085 && info->runNum <= 166967) {
            myTrigObj = kHLTObject_Ele32_CaloIdVT_CaloIsoT_TrkIdT_TrkIsoT;       
            myTrigBit = kHLT_Ele27_CaloIdVT_CaloIsoT_TrkIdT_TrkIsoT;
          } else if (info->runNum >= 166968 && info->runNum <= 170053) {
            myTrigObj = kHLTObject_Ele52_CaloIdVT_TrkIdT;       
            myTrigBit = kHLT_Ele27_CaloIdVT_CaloIsoT_TrkIdT_TrkIsoT;
          }
          else if (info->runNum >= 170054 && info->runNum <= 178380) {
            myTrigObj = kHLTObject_Ele65_CaloIdVT_TrkIdT;       
            myTrigBit = kHLT_Ele27_CaloIdVT_CaloIsoT_TrkIdT_TrkIsoT;
          } 
          else if (info->runNum > 178380 ) {
            myTrigObj = kHLTObject_Ele80_CaloIdVT_TrkIdT;       
            myTrigBit = kHLT_Ele27_CaloIdVT_CaloIsoT_TrkIdT_TrkIsoT;
          }
