//root -l EWKAna/Hww/Efficiency/ElectronTagAndProbeDebug.C+\(\"/home/sixie/hist/WWAnalysis/WWAnalysis_ele_noskim.root\",\"\"\)
...
void ElectronTagAndProbeDebug(const string dataInputFilename, const string Label) {   
...
//     if(hasJSON && !rlrm.HasRunLumi(rl)) continue;  // not certified run? Skip to next event...    
    ...
    if (info->evtNum == 40044772 || info->evtNum == 56182356 || info->evtNum == 56919694 || info->evtNum == 67544200|| info->evtNum == 228586|| info->evtNum == 18386566|| info->evtNum == 99004423|| info->evtNum == 100810704|| info->evtNum == 71682555|| info->evtNum == 126240777) printDebug = kTRUE;

    if (printDebug) {
      cout << "Check " << info->runNum << " " << info->lumiSec << " " <<   info->evtNum          
           << endl;
      
      for(Int_t i=0; i<electronArr->GetEntries(); i++) {
        const mithep::TElectron *ele = (mithep::TElectron*)((*electronArr)[i]);
        cout << "Electron " << i << " " << ele->pt << " " << ele->eta << " " << ele->phi << " : " << passElectronCuts(ele) 
             << " : " 
             << " " << ele->sigiEtaiEta
             << " " << ele->deltaEtaIn
             << " " << ele->deltaPhiIn
             << " " << ele->HoverE
             << " " << ((fabs(ele->eta) < 1.5)?(ele->trkIso03 + TMath::Max(ele->emIso03 - 1.0, 0.0) + ele->hadIso03) / ele->pt : (ele->trkIso03 + ele->emIso03 + ele->hadIso03) / ele->pt)
             << " " << ele->trkIso03 
             << " " << ele->emIso03 
             << " " << ele->hadIso03
             << " " << ele->d0
    ...
    for(Int_t i=0; i<electronArr->GetEntries(); i++) {
      ...
      for(Int_t j=0; j<electronArr->GetEntries(); j++) {
        ...
        if ( (0==0)
          ...
          if (printDebug) {
            cout << "Mass: " << dilepton.M() << endl;
          }

          if (passElectronCuts(probe)) {	 
            WriteMassToFile(outputFile_TagPlusRecoPassWWTightIdIso, dilepton.M(), info->evtNum );
            TagPlusRecoPassWWTightIdIso->Fill(dilepton.M());	      		  	      
          } else {
            WriteMassToFile(outputFile_TagPlusRecoFailWWTightIdIso, dilepton.M(), info->evtNum);
          ...
          if (passElectronCuts(probe)) {
            if (passHLT(probe, info->runNum)) {	 
              WriteMassToFile(outputFile_TagPlusWWTightIdIsoPassHLT, dilepton.M(), info->evtNum);
