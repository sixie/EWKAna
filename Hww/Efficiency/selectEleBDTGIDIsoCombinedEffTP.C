...
void selectEleBDTGIDIsoCombinedEffTP(const TString conf,              // input file
                                     const TString outputDir,         // output directory
                                     const Bool_t  matchGen = kFALSE  // match to generator muons
  ) {
  gBenchmark->Start("selectEleBDTGIDIsoCombinedEffTP");
  ...
  for(UInt_t ifile=0; ifile<fnamev.size(); ifile++) {  
	...
	if(fabs(tag->scEta) > 2.5) continue;
        ...
        if (!passElectronMVAIDIsoCombined(tag, electronIDMVA->MVAValue(
	...
	for(Int_t j=0; j<electronArr->GetEntriesFast(); j++) {
	  ...
	  nProbes++;
 	  ...
 	  Bool_t pass = 
            passElectronMVAIDIsoCombined(probe, electronIDMVA->MVAValue(
