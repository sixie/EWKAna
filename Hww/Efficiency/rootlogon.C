...
//     gSystem->Load("$CMSSW_BASE/lib/slc5_amd64_gcc434/libHiggsAnaNtupler.so");
    gSystem->Load("$CMSSW_BASE/lib/slc5_amd64_gcc434/libEWKAnaNtupler.so");
    
    gROOT->Macro("$CMSSW_BASE/src/Common/CPlot.cc+");
    gROOT->Macro("$CMSSW_BASE/src/Common/MitStyleRemix.cc+"); 
    gROOT->Macro("LeptonScaleLookup.cc+");
    ...
    gROOT->Macro(mypath+TString("/Common/CEffUser1D.cc+"));
    gROOT->Macro(mypath+TString("/Common/CEffUser2D.cc+")); 
  }
