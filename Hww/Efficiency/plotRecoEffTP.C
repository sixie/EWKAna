...
void plotRecoEffTP(const TString conf      = "zee.conf",      // input file
  ...
  // Set up histograms
  // 
  TH1F *hNumerPt    = new TH1F("hNumerPt","",50,elePtMin,elePtMax);
  TH1F *hDenomPt    = new TH1F("hDenomPt","",50,elePtMin,elePtMax);
  TH1F *hNumerEta   = new TH1F("hNumerEta","",50,eleEtaMin,eleEtaMax);
  TH1F *hDenomEta   = new TH1F("hDenomEta","",50,eleEtaMin,eleEtaMax);
  TH1F *hNumerPhi   = new TH1F("hNumerPhi","",40,-3.2,3.2);
  TH1F *hDenomPhi   = new TH1F("hDenomPhi","",40,-3.2,3.2);
  TH2F *hNumerPtEta = new TH2F("hNumerPtEta","",50,elePtMin,elePtMax,50,eleEtaMin,eleEtaMax);
  TH2F *hDenomPtEta = new TH2F("hDenomPtEta","",50,elePtMin,elePtMax,50,eleEtaMin,eleEtaMax);   

//TH1F *hMassPass   = new TH1F("hMassPass","",10,60,120);
//TH1F *hMassProbes = new TH1F("hMassProbes","",10,60,120);
  ...
  Double_t numerEB=0, denomEB=0;
  Double_t numerEE=0, denomEE=0;  
  ...
  for(UInt_t ifile=0; ifile<fnamev.size(); ifile++) {  
	...
	const Bool_t tagMatch1 = (toolbox::deltaR(gen->eta_1, gen->phi_1, tag->scEta, tag->scPhi)<dRMax);
	const Bool_t tagMatch2 = (toolbox::deltaR(gen->eta_2, gen->phi_2, tag->scEta, tag->scPhi)<dRMax);
	if(!tagMatch1 && !tagMatch2) continue;
...
//	  if(fabs(tag->deltaEtaIn) > 0.007)           continue;
	...
	for(Int_t j=0; j<electronArr->GetEntriesFast(); j++) {
	  ...
	  if(!(probe->isEcalDriven))   continue;  
          ...
          const Bool_t probeMatch1 = (toolbox::deltaR(gen->eta_1, gen->phi_1, probe->scEta, probe->scPhi)<dRMax);
	  const Bool_t probeMatch2 = (toolbox::deltaR(gen->eta_2, gen->phi_2, probe->scEta, probe->scPhi)<dRMax);
	  if(!probeMatch1 && !probeMatch2) continue;
	  if(!(tagMatch1 && probeMatch2) && !(tagMatch2 && probeMatch1)) continue;
	  	    	
          Bool_t pass = kTRUE;
#ifdef __WP95__		  
          if(probe->nExpHitsInner > 1) pass=kFALSE;	                  
	  if(probeEB) {
	    if(probe->trkIso03         > 0.15*(probe->pt)) pass=kFALSE;
	    if(probe->emIso03          > 2.00*(probe->pt)) pass=kFALSE;
	    if(probe->hadIso03         > 0.12*(probe->pt)) pass=kFALSE;
	    if(probe->sigiEtaiEta      > 0.01)             pass=kFALSE;
//	    if(fabs(probe->deltaPhiIn) > 0.8)              pass=kFALSE;
	    if(fabs(probe->deltaEtaIn) > 0.007)            pass=kFALSE;
	    if(probe->HoverE           > 0.15)             pass=kFALSE;
	  } else {
	    if(probe->trkIso03         > 0.08*(probe->pt)) pass=kFALSE;
	    if(probe->emIso03          > 0.06*(probe->pt)) pass=kFALSE;
	    if(probe->hadIso03         > 0.05*(probe->pt)) pass=kFALSE;
	    if(probe->sigiEtaiEta      > 0.03)             pass=kFALSE;
//	    if(fabs(probe->deltaPhiIn) > 0.7)              pass=kFALSE;
//	    if(fabs(probe->deltaEtaIn) > 0.001)            pass=kFALSE;
	    if(probe->HoverE           > 0.07)             pass=kFALSE;	  
	  }
#endif
#ifdef __WP80__
          if(probe->nExpHitsInner > 0) pass=kFALSE;
	  if((fabs(probe->partnerDist)<0.02) && (fabs(probe->partnerDeltaCot)<0.02)) pass=kFALSE; 	        
      	  if(probeEB) {
	    if(probe->trkIso03         > 0.09*(probe->pt)) pass=kFALSE;
	    if(probe->emIso03          > 0.07*(probe->pt)) pass=kFALSE;
	    if(probe->hadIso03         > 0.10*(probe->pt)) pass=kFALSE;
	    if(probe->sigiEtaiEta      > 0.01)             pass=kFALSE;
	    if(fabs(probe->deltaPhiIn) > 0.06)             pass=kFALSE;
	    if(fabs(probe->deltaEtaIn) > 0.004)            pass=kFALSE;
	    if(probe->HoverE           > 0.04)             pass=kFALSE;
	  } else {
	    if(probe->trkIso03         > 0.04*(probe->pt))  pass=kFALSE;
	    if(probe->emIso03          > 0.05*(probe->pt))  pass=kFALSE;
	    if(probe->hadIso03         > 0.025*(probe->pt)) pass=kFALSE;
	    if(probe->sigiEtaiEta      > 0.03)              pass=kFALSE;
	    if(fabs(probe->deltaPhiIn) > 0.03)              pass=kFALSE;
//	    if(fabs(probe->deltaEtaIn) > 0.007)             pass=kFALSE;
	    if(probe->HoverE           > 0.025)             pass=kFALSE;	  
	  }  
#endif   
...
//if((!probeMatch1 && !probeMatch2) || (!(tagMatch1 && probeMatch2) && !(tagMatch2 && probeMatch1))) {
//  hMassProbes->Fill(vDiEle.M());
//  if(pass) hMassPass->Fill(vDiEle.M());
//  continue;
//}
//cout << info->evtNum;
...
//else     cout << " FAIL" << endl;
	  ...
	  outTree->Fill();
          ...
          if(probe->scEt>=etCut && fabs(probe->scEta)<=etaCut) { 
            denom+=weight; if(pass) numer+=weight; 
	    if(probeEB) { denomEB+=weight; if(pass) numerEB+=weight; }
	    else        { denomEE+=weight; if(pass) numerEE+=weight; }
          }
	}
      }
    }
  ...
  Double_t errlEB, errhEB;  
  Double_t effEB = toolbox::calcEff(numerEB,denomEB,&errlEB,&errhEB,method);
  Double_t errlEE, errhEE;  
  Double_t effEE = toolbox::calcEff(numerEE,denomEE,&errlEE,&errhEE,method);
...
/*
hMassProbes->SetXTitle("m(ee) [GeV/c^{2}]");
hMassProbes->SetYTitle("Events / 6.0 GeV/c^{2}");
hMassProbes->SetTitle("Probes-Unmatched (#DeltaR=0.9)");
hMassProbes->SetLineStyle(kDashed);
hMassProbes->SetLineColor(kRed);
hMassProbes->Draw();
c->SaveAs("probes-unmatchedR09.png");

hMassPass->SetXTitle("m(ee) [GeV/c^{2}]");
hMassPass->SetYTitle("Events / 6.0 GeV/c^{2}");
hMassPass->SetTitle("Pass-Unmatched (#DeltaR=0.9)");
hMassPass->Draw();
c->SaveAs("pass-unmatchedR09.png");
*/
  ...
  cout << "*--------------------------------------------------" << endl;
  cout << endl;
  cout << "   electron efficiency (pT>" << etCut << ", |eta|<" << etaCut << "):" << endl;
  cout << "      " << numer << " / " << denom << " = " << eff << " (+" << errh << ", -" << errl << ")" << endl;
  cout << "      " << numerEB << " / " << denomEB << " = " << effEB << " (+" << errhEB << ", -" << errlEB << ")" << endl;
  cout << "      " << numerEE << " / " << denomEE << " = " << effEE << " (+" << errhEE << ", -" << errlEE << ")" << endl;
  cout << endl;
  if(lumi>0) {
  ...
  txtfile << "   electron efficiency (pT>" << etCut << ", |eta|<" << etaCut << "):" << endl;
  txtfile << "      " << numer << " / " << denom << " = " << eff << " (+" << errh << ", -" << errl << ")" << endl;
  txtfile << "      " << numerEB << " / " << denomEB << " = " << effEB << " (+" << errhEB << ", -" << errlEB << ")" << endl;
  txtfile << "      " << numerEE << " / " << denomEE << " = " << effEE << " (+" << errhEE << ", -" << errlEE << ")" << endl;
  txtfile << endl; 
  txtfile.close();
