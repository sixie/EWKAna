...
void selectMuEGElectronTrailingLegTrigEffTP(const TString conf,              // input file
	...
	for(Int_t j=0; j<electronArr->GetEntriesFast(); j++) {
	  ...
	  if(matchGen) {
	    ...
	    if(!match1 && !match2)
          ...
          if (!passElectronMVA(probe, electronIDMVAWithIPInfo->MVAValue(
...
//           if (NJets != 2 && NBTags >= 1) continue;
	  ...
	  if(info->runNum >= 150000 && info->runNum <= 173198) pass = pass || ((info->triggerBits & kHLT_Mu17_Ele8_CaloIdL)           
                                                                               //&& (probe->hltMatchBits & kHLTObject_Ele8_CaloIdL)
            );	  
	  if(info->runNum >= 173199 && info->runNum <= 999999) pass = pass || ((info->triggerBits & kHLT_Mu17_Ele8_CaloIdT_CaloIsoVL) 
                                                                               //&& (probe->hltMatchBits & kHLTObject_Ele8_CaloIdT_CaloIsoVL)
            );	  
	  if(typev[ifile]==eMC) 
