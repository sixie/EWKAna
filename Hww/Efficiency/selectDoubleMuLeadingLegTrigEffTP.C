...
void selectDoubleMuLeadingLegTrigEffTP(const TString conf,              // input file
  ...
  for(UInt_t ifile=0; ifile<fnamev.size(); ifile++) {  
	...
	for(Int_t j=i+1; j<muonArr->GetEntriesFast(); j++) { 
	  ...
	  if(info->runNum >= 165085 && info->runNum <= 178380)
	    pass = (info->triggerBits & kHLT_Mu17_Mu8) && (probe->hltMatchBits & (kHLTObject_Mu13));	  
	  if(info->runNum >= 178381 )
	    pass = (info->triggerBits & kHLT_Mu17_Mu8) && (probe->hltMatchBits & (kHLTObject_Mu17));	  
          if(typev[ifile]==eMC)
	    pass = (info->triggerBits & kHLT_DoubleMu7) && (probe->hltMatchBits & kHLTObject_Mu7);
