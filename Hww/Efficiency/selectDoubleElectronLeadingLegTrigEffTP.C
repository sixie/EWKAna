...
void selectDoubleElectronLeadingLegTrigEffTP(const TString conf,            // input file
	...
	for(Int_t j=0; j<electronArr->GetEntriesFast(); j++) {
	  ...
	  nProbes += weight;
          ...
          if (info->runNum <= 170053) {
            myTrigObj = kHLTObject_Ele17_CaloIdL_CaloIsoVL;          
            myTrigBit = kHLT_Ele17_CaloIdL_CaloIsoVL_Ele8_CaloIdL_CaloIsoVL;
          } else if (info->runNum <= 999999) {
            myTrigObj = kHLTObject_Ele17_CaloIdT_TrkIdVL_CaloIsoVL_TrkIsoVL;       
