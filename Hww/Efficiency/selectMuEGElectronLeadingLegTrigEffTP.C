...
void selectMuEGElectronLeadingLegTrigEffTP(const TString conf,              // input file
...
//           if (NJets != 2 && NBTags >= 1) continue;
	  ...
	  if(info->runNum >  170053 && info->runNum <= 173199) pass = pass || ( (info->triggerBits & kHLT_Mu8_Ele17_CaloIdT_CaloIsoVL)
                                                                               //&& (probe->hltMatchBits & kHLTObject_Ele17_CaloIdT_CaloIsoVL)
            );
	  if(info->runNum >  173199                          ) pass = pass || ( (info->triggerBits & kHLT_Mu8_Ele17_CaloIdT_CaloIsoVL)
                                                                               //&& (probe->hltMatchBits & kHLTObject_Ele17_CaloIdT_CaloIsoVL)
            );
	  if(typev[ifile]==eMC) 
