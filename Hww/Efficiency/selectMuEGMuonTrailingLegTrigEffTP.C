...
void selectMuEGMuonTrailingLegTrigEffTP(const TString conf,              // input file
...
//                  << endl;
	  ...
	  Bool_t pass = kFALSE;
	  if(info->runNum >= 150000 && info->runNum <= 170053) pass = pass || ( ((info->triggerBits & kHLT_Mu8_Ele17_CaloIdL) || (info->triggerBits & kHLT_Mu17_Ele8_CaloIdL))         
                                                                               // && (probe->hltMatchBits & kHLTObject_Ele17_CaloIdL)
            );	  
	  if(info->runNum >  170053 && info->runNum <= 173199) pass = pass || ( ((info->triggerBits & kHLT_Mu8_Ele17_CaloIdT_CaloIsoVL) || (info->triggerBits & kHLT_Mu17_Ele8_CaloIdL) )
                                                                               //&& (probe->hltMatchBits & kHLTObject_Ele17_CaloIdT_CaloIsoVL)
            );
	  if(info->runNum >  173199                          ) pass = pass || ( ((info->triggerBits & kHLT_Mu8_Ele17_CaloIdT_CaloIsoVL) || (info->triggerBits & kHLT_Mu17_Ele8_CaloIdT_CaloIsoVL))
                                                                               //&& (probe->hltMatchBits & kHLTObject_Ele17_CaloIdT_CaloIsoVL)
            );
	  if(typev[ifile]==eMC) 
