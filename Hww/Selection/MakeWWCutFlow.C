//root -l EWKAna/Hww/Selection/MakeWWCutFlow.C+\(\"\",0\)
...
void MakeWWCutFlow(const string Label, Int_t ElectronSelectionType) 
{  
...
//   inputFiles.push_back(vector<string>());
//    inputFiles.back().push_back("/home/sixie/hist/HwwAnalysis/normalized/HwwAnalysis_f10-h130ww2l-gf-z2-v12-pu_noskim_normalized.root");
...
//   inputFiles.back().push_back("/home/sixie/hist/HwwAnalysis/normalized/WWAnalysis_w10-ggww-z2-v8-pu11_noskim_normalized.root");
//   inputFiles.push_back(vector<string>());
//   inputFiles.back().push_back("/home/sixie/hist/HwwAnalysis/normalized/WWAnalysis_w10-wz-z2-v8-pu11_noskim_normalized.root");
//   inputFiles.back().push_back("/home/sixie/hist/HwwAnalysis/normalized/WWAnalysis_w10-zz-z2-v8-pu11_noskim_normalized.root");
//   inputFiles.push_back(vector<string>());
//   inputFiles.back().push_back("/home/sixie/hist/HwwAnalysis/normalized/WWAnalysis_w10-tt-mg-z2-v8-pu11_noskim_normalized.root");
//   inputFiles.back().push_back("/home/sixie/hist/HwwAnalysis/normalized/WWAnalysis_w10-stop-mg-z2-v8-pu11_noskim_normalized.root");
//   inputFiles.back().push_back("/home/sixie/hist/HwwAnalysis/normalized/WWAnalysis_w10-ttop-mg-z2-v8-pu11_noskim_normalized.root");
//   inputFiles.back().push_back("/home/sixie/hist/HwwAnalysis/normalized/WWAnalysis_w10-wtop-mg-z2-v8-pu11_noskim_normalized.root");
//   inputFiles.push_back(vector<string>());
//   inputFiles.back().push_back("/home/sixie/hist/HwwAnalysis/normalized/WWAnalysis_w10-zmm-powheg-c10-v8-pu11_noskim_normalized.root");
//   inputFiles.back().push_back("/home/sixie/hist/HwwAnalysis/normalized/WWAnalysis_w10-ztt-powheg-c10-v8-pu11_noskim_normalized.root");
//   inputFiles.back().push_back("/home/sixie/hist/HwwAnalysis/normalized/WWAnalysis_w10-zmm1020-powheg-c10-v8-pu11_noskim_normalized.root");
//   inputFiles.back().push_back("/home/sixie/hist/HwwAnalysis/normalized/WWAnalysis_w10-ztt1020-powheg-c10-v8-pu11_noskim_normalized.root");
//    inputFiles.push_back(vector<string>());
//    inputFiles.back().push_back("/home/sixie/hist/HwwAnalysis/normalized/HwwAnalysis_w10-wjetsl-z2-v8-pu11-skimmed_noskim_normalized.root");

  vector<string> processNames;
//   processNames.push_back("Data");
//   processNames.push_back("HWW");
...
//         eventweight = 1;
     ...
     LikelihoodMeasurements measurements;
      ...
      double likelihood = LH->result(measurements);
          ...
          if ( (0==0)
               && 
               passElectronCuts(ele, ElectronSelectionType, likelihood)
               &&
               fabs(ele->eta) < 2.5
               && 
               ele->pt > 0.0
        ...
        for(int i = 0; i < leptonPt.size(); ++i) {
          for(int j = i+1; j < leptonPt.size(); ++j) {
            ...
            //pt cuts
            if (!(leptonPt[i] > 20.0 && leptonPt[j] > 20.0)) continue;
...
//              if (!(lepton2.Pt() < 20 && lepton2.Pt() >= 15)) continue;
//               if (!(lepton2.Pt() >= 20)) continue;
  ...
  //============================================================================================================== 
  std::ofstream texFile("HwwCutFlowTable.tex");
...
//   texFile << "  Cut & " << processNames[0] << " & " << processNames[1] << " & " << processNames[2] << " & " << processNames[3] << " & " << processNames[4] << " & " << processNames[5] << " & " << processNames[5] << " & " << processNames[6] << " \\" << endl;
  ...
  for(int c=0; c<CutLabel.size() ; ++c) {
    ...
    for (int q = 0; q<processNames.size() ; ++q) { 
      char tmp[20];
         sprintf(tmp, "%.4f", fHWWSelection[q]->GetBinContent(1+c));
...
//mva
  if (ElectronSelectionType == 503) {
    if (ele->mva < 0.4) pass = kFALSE;
  }

  if (ElectronSelectionType == 504) {
    if (ele->mva < 0.5) pass = kFALSE;
  }

  if (ElectronSelectionType == 505) {
    if (ele->mva < 0.6) pass = kFALSE;
  }

  if (ElectronSelectionType == 506) {
    if (ele->mva < 0.7) pass = kFALSE;
  }

  if (ElectronSelectionType == 507) {
    if (ele->mva < 0.8) pass = kFALSE;
  }
