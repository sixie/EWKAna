//root -l EWKAna/Hww/FakeLeptonBkg/HwwFakeMuonPrediction_MC.C+\(\"/home/sixie/hist/HwwAnalysis/normalized/HwwAnalysis_w10-wjets-z2-v8-pu11-2l_noskim_normalized.root\",130,\"\"\)
//================================================================================================
//
// HWW selection macro
//
...
void HwwFakeMuonPrediction_MC(const string inputFilename, Double_t mHiggs, 
                 const string Label) 
{  
...
//                                                           "ElectronFakeRate.SmurfV4TTruncated20.root",
                                                     ...
                                                     "MuonFakeRateDenominatorM2_QCDMCCombined_ptThreshold15_PtEta",
//                                                      "MuonFakeRateDenominatorM2_Mu8Sample_ptThreshold15_PtEta",
  ...
  TH1D *fDeltaPhiLeptons_WWSelection_predicted = new TH1D(    "hDeltaPhiLeptons_WWSelection_predicted",";#Delta#phi_{ll};Number of Events",9,0,180);
  ...
  TH1D *dileptonMass_WWSelection_predicted = new TH1D("dileptonMass_WWSelection_predicted", "; Mass [GeV/c^{2}]; Number of Events", 10, 0, 200);
  ...
  TH1D *fDeltaPhiLeptons_WWSelection_fullSim = new TH1D(    "hDeltaPhiLeptons_WWSelection_fullSim",";#Delta#phi_{ll};Number of Events",9,0,180);
  ...
  TH1D *dileptonMass_WWSelection_fullSim = new TH1D("dileptonMass_WWSelection_fullSim", "; Mass [GeV/c^{2}]; Number of Events", 10, 0, 200);
  ...
  for(UInt_t ientry=0; ientry<eventTree->GetEntries(); ientry++) {       	
    ...
    if (!(leptonPt.size() < 1) && (leptonPt[0] > 10.0) 
      ...
      if (!(mu->pt > 10.0 && fabs(mu->eta) < 2.5)) continue;
        if ( mithep::MathUtils::DeltaR(mu->phi, mu->eta, leptonPhi[0],leptonEta[0]) < 0.1 )
          continue;
        ...
        //preselection
        if (!(mu->pt > 10.0 && fabs(mu->eta) < 2.5)) continue;
        if ((ChargeSelection == 0 && leptonCharge[0] == mu->q) || (ChargeSelection == 1 && leptonCharge[0] != mu->q)) continue;
        ...
        //Calculate Fake Rate
        //******************************************************************************
        double fakeRate = fFakeRate->MuonFakeRate(TMath::Min(double(mu->pt),double(34.5)), fabs(mu->eta), mu->phi) / (1-fFakeRate->MuonFakeRate(TMath::Min(double(mu->pt),double(34.5)), fabs(mu->eta), mu->phi));
        double fakeRateErrorLow = fFakeRate->MuonFakeRateStatErrorLow(TMath::Min(double(mu->pt),double(34.5)), fabs(mu->eta), mu->phi) / pow((1- fFakeRate->MuonFakeRate(TMath::Min(double(mu->pt),double(34.5)), fabs(mu->eta), mu->phi)),2);
        double fakeRateErrorHigh = fFakeRate->MuonFakeRateStatErrorHigh(TMath::Min(double(mu->pt),double(34.5)), fabs(mu->eta), mu->phi) / pow((1- fFakeRate->MuonFakeRate(TMath::Min(double(mu->pt),double(34.5)), fabs(mu->eta), mu->phi)),2);
        ...
        // eventWeightError = 0;
        if (fFakeRate->MuonFakeRate(TMath::Min(double(mu->pt),double(34.5)), fabs(mu->eta), mu->phi) == 1) {
          eventWeight = 1;
          eventWeightError = 1;
        }
       ...
       fakeLepton.SetCoordinates(mu->pt, mu->eta, mu->phi, 0.51099892e-3 );
       mithep::FourVectorM fakepfLepton;
       fakepfLepton.SetCoordinates(mu->pfPt, mu->pfEta, mu->pfPhi, 0.51099892e-3 );
       if (mu->pfPt >= 0) {
       ...
       if  (printDebug)cout << "Corrected Met : " << pfTrackMet.Px() << " , " << pfTrackMet.Py() << " : " << pfTrackMet.Pt() << endl;
        ...
        for(Int_t j=0; j<muonArr->GetEntries(); j++) {
          const mithep::TMuon *softMu = (mithep::TMuon*)((*muonArr)[j]);
          Bool_t isCleanMuon = kFALSE;        
          if ( leptonType[0] == 13 
               && mithep::MathUtils::DeltaR(softMu->phi, softMu->eta, leptonPhi[0],leptonEta[0]) < 0.1
            ) {
            isCleanMuon = kTRUE;         
          }
          Bool_t overlapWithDenominator = kFALSE;
          if ( mithep::MathUtils::DeltaR(softMu->phi, softMu->eta, mu->phi, mu->eta) < 0.3
            ) {
            overlapWithDenominator = kTRUE;         
          }
          if ( softMu->pt > 3.0
               && (softMu->typeBits & kTracker)
               && (softMu->qualityBits & kTMLastStationAngTight)
               && softMu->nTkHits > 10
               && fabs(softMu->d0) < 0.2
               && fabs(softMu->dz) < 0.1
               && !isCleanMuon
               && (!(softMu->pt > 20 && (softMu->trkIso03 + softMu->emIso03 + softMu->hadIso03) / mu->pt < 0.10))
               && !overlapWithDenominator
            ) {
            NSoftMuons++;
          }
        }
        ...
        if (leptonPt[0] > mu->pt) {
          ...
          lepton2.SetCoordinates(mu->pt, mu->eta, mu->phi, 105.658369e-3 );
        } else {
          if (leptonType[0] == 11) {
            finalState = 3;
          ...
          lepton1.SetCoordinates(mu->pt, mu->eta, mu->phi, 105.658369e-3 );
        }
        ...
        if (printDebug) {
          ...
          cout << mu->pt << " " << mu->eta << " " << mu->phi << " " << "11" << endl;
        ...
        if (passCut[0] && passCut[1] && passCut[2] && passCut[3] ) {
          fLeptonFakePt_preSelection_predicted->Fill(mu->pt, eventWeight);
          fLeptonFakeEta_preSelection_predicted->Fill(mu->eta, eventWeight);
        }        
...
//         if (passAllCuts) {
          ...
          fLeptonFakePt_WWSelection_predicted->Fill(mu->pt, eventWeight);
          fLeptonFakeEta_WWSelection_predicted->Fill(mu->eta, eventWeight);
          fLeptonFakePtEta_WWSelection_predicted->Fill(mu->pt, mu->eta, eventWeight);
...
//              passAllCuts           
          ...
          Double_t ptCategory =  mu->pt;
          Double_t etaCategory = mu->eta;

          if (ptCategory > 30.0) {
            if (fabs(etaCategory) >= 0 && fabs(etaCategory) < 1.0) {
              ...
              Count_Pt30ToInf_EtaBin0_sysError_WWSelection_predicted += eventWeightError;
            } else if (fabs(etaCategory) >= 1.0 && fabs(etaCategory) < 1.5) {
              ...
              Count_Pt30ToInf_EtaBin1_sysError_WWSelection_predicted += eventWeightError;
            } else if (fabs(etaCategory) >= 1.5 && fabs(etaCategory) < 2.5) {
              ...
              Count_Pt30ToInf_EtaBin2_sysError_WWSelection_predicted += eventWeightError;
            }
            ...
            if (fabs(etaCategory) >= 0 && fabs(etaCategory) < 1.0) {
              ...
              Count_Pt25To30_EtaBin0_sysError_WWSelection_predicted += eventWeightError;
            } else if (fabs(etaCategory) >= 1.0 && fabs(etaCategory) < 1.5) {
              ...
              Count_Pt25To30_EtaBin1_sysError_WWSelection_predicted += eventWeightError;
            } else if (fabs(etaCategory) >= 1.5 && fabs(etaCategory) < 2.5) {
              ...
              Count_Pt25To30_EtaBin2_sysError_WWSelection_predicted += eventWeightError;
            }
            ...
            if (fabs(etaCategory) >= 0 && fabs(etaCategory) < 1.0) {
              ...
              Count_Pt20To25_EtaBin0_sysError_WWSelection_predicted += eventWeightError;
            } else if (fabs(etaCategory) >= 1.0 && fabs(etaCategory) < 1.5) {
              ...
              Count_Pt20To25_EtaBin1_sysError_WWSelection_predicted += eventWeightError;
            } else if (fabs(etaCategory) >= 1.5 && fabs(etaCategory) < 2.5) {
              ...
              Count_Pt20To25_EtaBin2_sysError_WWSelection_predicted += eventWeightError;
            }
            ...
            if (fabs(etaCategory) >= 0 && fabs(etaCategory) < 1.0) {
              ...
              Count_Pt15To20_EtaBin0_sysError_WWSelection_predicted += eventWeightError;
            } else if (fabs(etaCategory) >= 1.0 && fabs(etaCategory) < 1.5) {
              ...
              Count_Pt15To20_EtaBin1_sysError_WWSelection_predicted += eventWeightError;
            } else if (fabs(etaCategory) >= 1.5 && fabs(etaCategory) < 2.5) {
              ...
              Count_Pt15To20_EtaBin2_sysError_WWSelection_predicted += eventWeightError;
            }
            ...
            if (fabs(etaCategory) >= 0 && fabs(etaCategory) < 1.0) {
              ...
              Count_Pt10To15_EtaBin0_sysError_WWSelection_predicted += eventWeightError;
            } else if (fabs(etaCategory) >= 1.0 && fabs(etaCategory) < 1.5) {
              ...
              Count_Pt10To15_EtaBin1_sysError_WWSelection_predicted += eventWeightError;
            } else if (fabs(etaCategory) >= 1.5 && fabs(etaCategory) < 2.5) {
              ...
              Count_Pt10To15_EtaBin2_sysError_WWSelection_predicted += eventWeightError;
            }
          } 
          ...
          if (leptonType[0] == 11) {
            if (mu->pt > 20) {
              if (fabs(mu->eta) < 1.479) {
                ...
                Count_eF_Pt20ToInf_Barrel_sysError_WWSelection_predicted += eventWeightError;
              } else {
                ...
                Count_eF_Pt20ToInf_Endcap_sysError_WWSelection_predicted += eventWeightError;
              }
            } else {
              if (fabs(mu->eta) < 1.479) {
                ...
                Count_eF_Pt10To20_Barrel_sysError_WWSelection_predicted += eventWeightError;
              } else {
                ...
                Count_eF_Pt10To20_Endcap_sysError_WWSelection_predicted += eventWeightError;
              }
            }
          } else {
            if (mu->pt > 20) {
              if (fabs(mu->eta) < 1.479) {
                ...
                Count_mF_Pt20ToInf_Barrel_sysError_WWSelection_predicted += eventWeightError;
              } else {
                ...
                Count_mF_Pt20ToInf_Endcap_sysError_WWSelection_predicted += eventWeightError;
              }
            } else {
              if (fabs(mu->eta) < 1.479) {
                ...
                Count_mF_Pt10To20_Barrel_sysError_WWSelection_predicted += eventWeightError;
              } else {
                ...
                Count_mF_Pt10To20_Endcap_sysError_WWSelection_predicted += eventWeightError;
              }
            }
          }
    ...
    if (
      !(leptonPt.size() < 2) 
        ...
        for(int i = 0; i < leptonPt.size(); ++i) {
          ...
          Bool_t isFakeMu = kFALSE;
          if (RealLeptonIndex == i) {
            ptCategory = leptonPt[j];
            etaCategory = leptonEta[j];
            if (leptonType[j] == 13) isFakeMu = kTRUE;
          } else {
            ptCategory = leptonPt[i];  
            etaCategory = leptonEta[i];
            if (leptonType[i] == 13) isFakeMu = kTRUE;
          }
          if (!isFakeMu) continue;
...
//   TFile *file = new TFile("WWSelectionPlots_IsoDenominator.root", "RECREATE");
  ...
  //*****************************************************************************************
  file = new TFile("MuonFakeBkgClosureTestPlots.root", "RECREATE");
  ...
  fLeptonFakePt_preSelection_fullSim->GetYaxis()->SetRangeUser(0,120);
  ...
  legend->Draw();
  cv->SaveAs("FakeMuonClosureTest_FakeMuPt_PreSelection.gif");
  file->WriteTObject(cv , "LeptonFakePt_PreSelection", "WriteDelete");
  ...
  fLeptonFakeEta_preSelection_fullSim->GetYaxis()->SetRangeUser(0,30);
  ...
  legend->Draw();
  cv->SaveAs("FakeMuonClosureTest_FakeMuEta_PreSelection.gif");
  file->WriteTObject(cv , "LeptonFakeEta_Preselection", "WriteDelete");
  ...
  legend->Draw();
  cv->SaveAs("FakeMuonClosureTest_FakeMuPt.gif");
  ...
  fLeptonFakeEta_WWSelection_fullSim->GetYaxis()->SetRangeUser(0,30);
  ...
  legend->Draw();
  cv->SaveAs("FakeMuonClosureTest_FakeMuEta.gif");
  ...
  fLeptonPtMax_WWSelection_fullSim->GetYaxis()->SetRangeUser(0,30);
  ...
  legend->Draw();
  cv->SaveAs("FakeMuonClosureTest_PtMax.gif");
  ...
  fLeptonPtMin_WWSelection_fullSim->GetYaxis()->SetRangeUser(0,60);
  ...
  legend->Draw();
  cv->SaveAs("FakeMuonClosureTest_PtMin.gif");
  ...
  fMetPtHist_WWSelection_fullSim->GetYaxis()->SetRangeUser(0,40);
  ...
  legend->Draw();
  cv->SaveAs("FakeMuonClosureTest_Met.gif");
  ...
  fDeltaPhiLeptons_WWSelection_fullSim->GetYaxis()->SetRangeUser(0,20);
  ...
  legend->Draw();
  cv->SaveAs("FakeMuonClosureTest_DeltaPhi.gif");
  ...
  legend->Draw();
  cv->SaveAs("FakeMuonClosureTest_DileptonMass.gif");
  ...
  legend->Draw();
  cv->SaveAs("FakeMuonClosureTest_MtHiggs.gif");
  ...
  legend->SetTextSize(0.03);
  legend->SetBorderSize(1);
  legend->AddEntry(fHWWToMuMuSelection_fullSim,"Simulation", "LP");
  legend->AddEntry(fHWWToMuMuSelection_predicted,"Predicted", "LP");
  fHWWToMuMuSelection_fullSim->SetMarkerColor(kBlue);
  fHWWToMuMuSelection_fullSim->SetLineColor(kBlue);
  fHWWToMuMuSelection_fullSim->GetYaxis()->SetTitleOffset(1.2);
  fHWWToMuMuSelection_fullSim->GetXaxis()->SetTitleOffset(1.05);
  fHWWToMuMuSelection_fullSim->GetYaxis()->SetRangeUser(0,120);
  fHWWToMuMuSelection_fullSim->Draw("E1");
  fHWWToMuMuSelection_predicted->SetMarkerColor(kRed);
  fHWWToMuMuSelection_predicted->SetLineColor(kRed);
  fHWWToMuMuSelection_predicted->SetFillColor(kRed);
  fHWWToMuMuSelection_predicted->SetFillStyle(3001);
  fHWWToMuMuSelection_predicted->Draw("E3,same");
  legend->Draw();
  cv->SaveAs("FakeMuonClosureTest_CutFlowMuMu.gif");
  file->WriteTObject(cv , "fHWWToMuMuSelection", "WriteDelete");
  ...
  fHWWToEMuSelection_fullSim->GetYaxis()->SetRangeUser(0,100);
  ...
  legend->Draw();
  cv->SaveAs("FakeMuonClosureTest_CutFlowEMu.gif");
