//root -l EWKAna/Hww/FakeLeptonBkg/HwwFakeMuonPrediction.C+\(\"/home/sixie/hist/HwwAnalysis/2011Data/WWAnalysisSkimmed_r11a-full-pr_TightPlusRecoTriggerSkim.root\",130,\"\"\) 
//root -l EWKAna/Hww/FakeLeptonBkg/HwwFakeMuonPrediction.C+\(\"/home/sixie/hist/HwwAnalysis/2011Data/WWAnalysisSkimmed_r11a-full-pr_LooseLooseTriggerSkim.root\",130,\"\"\)
//root -l EWKAna/Hww/FakeLeptonBkg/HwwFakeMuonPrediction.C+\(\"/home/sixie/hist/HwwAnalysis/2011Data/SmurfV6/WWAnalysisSkimmed_r11a-full-pr_TightPlusRecoTriggerSkim.root\",130,\"\"\)
//================================================================================================
//
// HWW selection macro
//
...
void HwwFakeMuonPrediction(const string inputFilename, Double_t mHiggs, 
                 const string Label) 
{  
...
//                                                          "MuonFakeRateDenominatorM1_Mu8Sample_ptThreshold20_PtEta",
...
//   hasJSON = kFALSE;
  ...
  for(UInt_t ientry=0; ientry<eventTree->GetEntries(); ientry++) {       	
    ...
    if (info->runNum == 163340 && info->evtNum == 15261232 ) {
      printDebug == kTRUE;
      cout << "HERE!!!\n";
    }
...
//     if (!passHLT(info->triggerBits, info->runNum, kTRUE)) continue;
    ...
    for(Int_t i=0; i<jetArr->GetEntries(); i++) {
      ...
      if (!(jet->pt > 30 && fabs(jet->eta) < 5.0)) continue;
    ...
    if (info->runNum == 163340 && info->evtNum == 15261232 ) {
      cout << "printDebug: " << printDebug << endl;
//     if (printDebug) {
...
//                  << " : " << passMuonCuts(mu) << " "
    ...
    //loop over muon denominators
    for(Int_t i=0; i<muonArr->GetEntries(); i++) {
      const mithep::TMuon *mu = (mithep::TMuon*)((*muonArr)[i]);
   

      if (!(mu->pt > 10.0 && fabs(mu->eta) < 2.4)) continue;
      ...
      for(Int_t jj=0; jj<jetArr->GetEntries(); jj++) {
        ...
        if ( mithep::MathUtils::DeltaR(mu->phi, mu->eta, jet->phi,jet->eta) < 0.5 ) {
          leptonJetPt = jet->pt;
        }                
      }
      if (leptonJetPt < 0) leptonJetPt = mu->pt;
      if (mu->pt < 30) {
...
//       eventWeight = 1.0; 
      ...
      for(Int_t j=0; j<jetArr->GetEntries(); j++) {
        ...
        if (mithep::MathUtils::DeltaR(jet->phi, jet->eta, leptonPhi[0],leptonEta[0]) < 0.5 ||
...
//       if(NJets     == 1)              passCut[6] = true;
      ...
      if (printDebug) {
        ...
        cout << mu->pt << " " << mu->eta << " " << mu->phi << " " << "13" << endl;
...
//         if (passAllCuts) {
        ...
        fLeptonFakePt->Fill(mu->pt, eventWeight);
        fLeptonFakeEta->Fill(mu->eta, eventWeight);
        fLeptonFakePtEta->Fill(mu->pt, mu->eta, eventWeight);
...
//              && passAllCuts           
        ...
        cout << info->runNum << " " << info->evtNum << " " << info->lumiSec << " : " << mu->pt << " " << mu->eta << " : " << fakeRate << " - " << fakeRateErrorLow << " + " << fakeRateErrorHigh << "  : " << finalState << endl;
...
//   TFile *file = new TFile("WWSelectionPlots_IsoDenominator.root", "RECREATE");
  ...
  file = new TFile("MuonFakeRate.SmurfV5.skim.root", "UPDATE");
