...
void AssociatedHiggsProduction()
{
   //Draw Feynman diagrams
   // To see the output of this macro, click begin_html <a href="gif/feynman.gif">here</a>. end_html
   //Author: Otto Schaile
   
   TCanvas *c1 = new TCanvas("c1", "A canvas", 10,10, 400, 300);
   c1->Range(0, 0, 95, 60);
   Int_t linsav = gStyle->GetLineWidth();
   gStyle->SetLineWidth(3);
   TLatex t;
   ...
   l = new TLine(10, 50, 30, 30); l->Draw();
   t.DrawLatex(7,6,"q");
   t.DrawLatex(7,55,"#bar{q}");
 
   TCurlyLine *VectorBoson = new TCurlyLine(30, 30, 60, 30);
   VectorBoson->SetWavy();
   VectorBoson->Draw();
   t.DrawLatex(42.5,37.7,"W,Z");


   TCurlyLine *OutGoingVectorBoson = new TCurlyLine(60, 30, 80, 10);
   OutGoingVectorBoson->SetWavy();
   OutGoingVectorBoson->Draw();
   l = new TLine(60, 30, 80, 50); l->Draw();
   l->SetLineStyle(7);

   t.DrawLatex(85,55,"H^{0}");
   t.DrawLatex(85,6,"W,Z");

   c1->Update();
   gStyle->SetLineWidth(linsav);

   c1->SaveAs("VHProductionDiagram.eps");

}

void FeynmanDiagram() {

  AssociatedHiggsProduction();
