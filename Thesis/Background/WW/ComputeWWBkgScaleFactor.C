...
void ComputeWWBkgScaleFactor (
  Int_t period = -1,
  char* bgdInputFile    = "/data/smurf/sixie/data/Thesis/Run2011_Summer11_SmurfV7_42X/mitf-alljets/backgroundC_skim1.root",
  char* dataInputFile   = "/data/smurf/sixie/data/Thesis/Run2011_Summer11_SmurfV7_42X/mitf-alljets/data_2l_skim1.root"
)
{
...
// ***********************************************************************************************
  ...
  if	 (period == 0){ // Run2011A
    effPath  = "/data/smurf/sixie/data/Thesis/auxiliar/Winter11_4700ipb/efficiency_results_v7_42x_Run2011A.root";
    fakePath = "/data/smurf/sixie/data/Thesis/auxiliar/Winter11_4700ipb/FakeRates_CutBasedMuon_BDTGWithIPInfoElectron.root";
    puPath   = "/data/smurf/sixie/data/Thesis/auxiliar/Winter11_4700ipb/PileupReweighting.Summer11DYmm_To_Run2011A.root";
    scaleFactorLum     = 2.1;minRun =      0;maxRun = 173692;
  }
  else if(period == 1){ // Run2011B
    effPath  = "/data/smurf/sixie/data/Thesis/auxiliar/Winter11_4700ipb/efficiency_results_v7_42x_Run2011B.root";
    fakePath = "/data/smurf/sixie/data/Thesis/auxiliar/Winter11_4700ipb/FakeRates_CutBasedMuon_BDTGWithIPInfoElectron.root";
    puPath   = "/data/smurf/sixie/data/Thesis/auxiliar/Winter11_4700ipb/PileupReweighting.Summer11DYmm_To_Run2011B.root";
    scaleFactorLum     = 1.9;minRun = 173693;maxRun = 999999;
  }
  else if(period == 2){ // Full2011
    effPath  = "/data/smurf/sixie/data/Thesis/auxiliar/Winter11_4700ipb/efficiency_results_v7_42x_Full2011_4700ipb.root";
    fakePath = "/data/smurf/sixie/data/Thesis/auxiliar/Winter11_4700ipb/FakeRates_CutBasedMuon_BDTGWithIPInfoElectron.root";
    puPath   = "/data/smurf/sixie/data/Thesis/auxiliar/Winter11_4700ipb/PileupReweighting.Summer11DYmm_To_Full2011.root";
    scaleFactorLum     = 4.7;minRun =      0;maxRun = 999999;
  }
  else if(period == 3){ // Full2011-Fall11
    effPath  = "/data/smurf/sixie/data/Thesis/auxiliar/Winter11_4700ipb/efficiency_results_v7_42x_Full2011_4700ipb.root";
    fakePath = "/data/smurf/sixie/data/Thesis/auxiliar/Winter11_4700ipb/FakeRates_CutBasedMuon_BDTGWithIPInfoElectron.root";
    puPath   = "/data/smurf/sixie/data/Thesis/auxiliar/Winter11_4700ipb/PileupReweighting.Fall11_To_Full2011.root";
...
//           passMassCut = ( dilep->mass() > DileptonMassPreselectionCut(mH[imass]));
  ...
  // Print Summary
  //********************************************************
  ofstream fResultTexTable("WWBkgEstimate.tex");
  char buffer[200];
  ...
  for(UInt_t classIndex = 0; classIndex < 4; ++classIndex) {
    ...
    fResultTexTable << "\\multicolumn{4}{|c|}{" << classLabel << " Analysis} \\\\" << endl;
    fResultTexTable << "\\hline" << endl;
    fResultTexTable << setw(35) << left << "$m_{\\mathrm{Higgs}}$ hypothesis"
               << setw(3) << left << "&"   
               << setw(15) << left << "WW yields (data)"
               << setw(3) << left << "&"   
               << setw(15) << left << "WW yields (MC)"
               << setw(3) << left << "&"   
    ...
    for(Int_t imass=0; imass<nmass; imass++) {
      ...
      sprintf(buffer,"%.1f +/- %.1f",Yield_WWControlRegion_Data[classIndex][imass] - Yield_WWControlRegion_Bkg[classIndex][imass],
              TMath::Sqrt(Yield_WWControlRegion_Data[classIndex][imass] + pow(YieldUncertainty_WWControlRegion_Bkg[classIndex][imass],2)));
      fResultTexTable << setw(15) << left << buffer;
      fResultTexTable << setw(3) << left << "&" ;

      sprintf(buffer,"%.1f +/- %.1f",Yield_WWControlRegion_WWMC[classIndex][imass],
              YieldUncertainty_WWControlRegion_WWMC[classIndex][imass]);
      fResultTexTable << setw(15) << left << buffer;
      fResultTexTable << setw(3) << left << "&" ;

      sprintf(buffer,"%.2f +/- %.2f",WWScaleFactor[classIndex][imass],
              WWScaleFactorUncertainty[classIndex][imass]);
      fResultTexTable << setw(15) << left << buffer;
      fResultTexTable << " \\\\" ;
